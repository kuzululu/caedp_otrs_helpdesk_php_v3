...
<section class="printAll">
 <div class="container-fluid">
   <div class="row">
     <div class="text-end">
        <a class="btn btn-outline-success btn-sm btn-printPendingAll" type="button"><i class="fa fa-print"></i></a>
     </div>
   </div>
 </div> 
</section>
...
<section class="print">
 <div class="container-fluid">
   <div class="row">
     <div class="text-end">
        <a class="btn btn-outline-success btn-sm btn-printPending" type="button"><i class="fa fa-print"></i></a>
     </div>
   </div>
 </div> 
</section>
...
?>  	 
  <tr class="text-center align-middle">
    ...
    <td class="align-top"><a target="_blank" href="../upload/<?= htmlspecialchars($row['upload_file']) ?>" class="text-success fst-italic fw-bolder"><?= htmlspecialchars(shortenLinkName($row["upload_file"])) ?></a></td>
  	<td>
  		<a href="pending_report?id=<?= htmlspecialchars(urlencode(base64_encode($row['id']))) ?>&&technician=<?= urlencode(base64_encode($_SESSION['first_name'] . ' ' . $_SESSION['last_name'])) ?>" class="btn btn-outline-secondary btn-sm"><i class="fa-regular fa-eye"></i></a> <a href="#" class="btn btn-outline-primary btn-sm uploadFile" id="<?= htmlspecialchars($row['id']) ?>" data-bs-toggle="modal" data-bs-target="#modalFileUpload"><i class="fa-regular fa-file"></i></a>
  	</td>
  </tr>
<?php
$ctr++;
...
<div class="table-responsive" id="pendingAllRecords">
 <table class="table table-hover">
   <thead>
     <tr class="text-center align-middle">
      <th>No.</th>
...
$allRecordsManager = new IncidentRecords($conn);
$totalAllRecords = $allRecordsManager->allPendingRecordsCount();

$recAll = $allRecordsManager->allPendingRecords();
